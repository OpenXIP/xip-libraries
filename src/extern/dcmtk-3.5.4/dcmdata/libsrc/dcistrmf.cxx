...
OFBool DcmFileProducer::eos() const
{
  if (file_)
  {
    // DCMTK 3.5.4 : return (feof(file_) || (size_ == OFstatic_cast(Uint32, ftell(file_))));
      return ((pos_ == size_) || (feof(file_)));
  }
  else return OFTrue;
}
...
{
  // DCMTK 3.5.4 : if (file_) return size_ - OFstatic_cast(Uint32, ftell(file_)); else return 0;
  if (file_) return size_ - pos_; else return 0;
}
...
Uint32 DcmFileProducer::read(void *buf, Uint32 buflen)
{
  ...
  if (status_.good() && file_ && buf && buflen)
  {
    ...
    // the following line added to DCMDK 3.5.4
    pos_ += result;
...
Uint32 DcmFileProducer::skip(Uint32 skiplen)
{
  ...
  {
    // DCMTK 3.5.4 : Uint32 pos = OFstatic_cast(Uint32, ftell(file_));
    result = (size_ - pos_ < skiplen) ? (size_ - pos_) : skiplen;
    if (fseek(file_, result, SEEK_CUR))
    {
       const char *text = strerror(errno);
       if (text == NULL) text = "(unknown error code)";
       status_ = makeOFCondition(OFM_dcmdata, 18, OF_error, text);
    } else // this else statement added to DCMTK 3.5.4
	{
	   pos_ += result;
	}
  }
