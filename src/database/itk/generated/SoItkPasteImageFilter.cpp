...
SO_ENGINE_SOURCE( SoItkPasteImageFilter );

SoItkPasteImageFilter::SoItkPasteImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkPasteImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( DestinationIndex, (0) );
	DestinationIndex.setNum(0);
	SO_ENGINE_ADD_INPUT( SourceRegion, (0) );
	SourceRegion.setNum(0);
...
{
	SO_ENGINE_INIT_CLASS( SoItkPasteImageFilter, SoItkInPlaceImageFilter, "SoItkInPlaceImageFilter" );
}

bool
SoItkPasteImageFilter::checkInput()
{
...
void
SoItkPasteImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input1.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::PasteImageFilter< OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_INDEX( filter, DestinationIndex, 2 );
						SO_ITK_SET_FIELD_IMAGE_REGION( filter, SourceRegion, 2 );
					...
					case 3:
					{
						...
						// Set the filter's inputs
						SO_ITK_SET_FIELD_INDEX( filter, DestinationIndex, 3 );
						SO_ITK_SET_FIELD_IMAGE_REGION( filter, SourceRegion, 3 );
