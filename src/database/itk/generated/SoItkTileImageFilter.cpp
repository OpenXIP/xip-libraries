...
SO_ENGINE_SOURCE( SoItkTileImageFilter );

SoItkTileImageFilter::SoItkTileImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkTileImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( Layout, (0) );
	Layout.setNum(0);
...
{
	SO_ENGINE_INIT_CLASS( SoItkTileImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkTileImageFilter::checkInput()
{
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						SO_ITK_SET_FIELD_FIXEDARRAY( filter, Layout, unsigned int, 2 );
					...
					case 3:
					{
						...
						// Set the filter's inputs
						SO_ITK_SET_FIELD_FIXEDARRAY( filter, Layout, unsigned int, 3 );
						SO_ITK_SET_FIELD_VALUE( filter, DefaultPixelValue );
