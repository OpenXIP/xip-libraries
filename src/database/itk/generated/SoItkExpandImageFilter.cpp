...
SO_ENGINE_SOURCE( SoItkExpandImageFilter );

SoItkExpandImageFilter::SoItkExpandImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkExpandImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( ExpandFactors, (0) );
	SO_ENGINE_ADD_INPUT( EdgePaddingValue, (0) );
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkExpandImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkExpandImageFilter::checkInput()
{
...
void
SoItkExpandImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::ExpandImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUE( filter, ExpandFactors );
						SO_ITK_SET_FIELD_VALUE( filter, EdgePaddingValue );
