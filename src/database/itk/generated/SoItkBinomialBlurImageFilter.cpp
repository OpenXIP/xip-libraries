...
SO_ENGINE_SOURCE( SoItkBinomialBlurImageFilter );

SoItkBinomialBlurImageFilter::SoItkBinomialBlurImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkBinomialBlurImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( Repetitions, (1) );
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkBinomialBlurImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkBinomialBlurImageFilter::checkInput()
{
...
void
SoItkBinomialBlurImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						SO_ITK_SET_FIELD_VALUE( filter, Repetitions );
