...
SO_ENGINE_SOURCE( SoItkTernaryMagnitudeSquaredImageFilter );

SoItkTernaryMagnitudeSquaredImageFilter::SoItkTernaryMagnitudeSquaredImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkTernaryMagnitudeSquaredImageFilter );
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkTernaryMagnitudeSquaredImageFilter, SoItkTernaryFunctorImageFilter, "SoItkTernaryFunctorImageFilter" );
}

bool
SoItkTernaryMagnitudeSquaredImageFilter::checkInput()
{
	switch( Input2.getValue()->getNumDimension() )
	{
		case 2:
		{
			switch( Input2.getValue()->getType() )
			{
...
void
SoItkTernaryMagnitudeSquaredImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case 2:
			{
				switch( Input2.getValue()->getType() )
				{
					case SoItkDataImage::FLOAT:
					{
						...
						typedef itk::TernaryMagnitudeSquaredImageFilter< InputImageType, InputImageType, InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						
						// Inherited from SoItkTernaryFunctorImageFilter
						typedef itk::Image< float, 2 > InputImageType;
						SO_ITK_SET_FIELD_MDATA( filter, Input, 1, InputImageType );
						typedef itk::Image< float, 2 > InputImageType;
						SO_ITK_SET_FIELD_MDATA( filter, Input, 2, InputImageType );
