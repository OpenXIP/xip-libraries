...
SO_ENGINE_SOURCE( SoItkNonThreadedShrinkImageFilter );

SoItkNonThreadedShrinkImageFilter::SoItkNonThreadedShrinkImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkNonThreadedShrinkImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( ShrinkFactors, (0) );
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkNonThreadedShrinkImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkNonThreadedShrinkImageFilter::checkInput()
{
...
void
SoItkNonThreadedShrinkImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::NonThreadedShrinkImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUE( filter, ShrinkFactors );
