...
SO_ENGINE_SOURCE( SoItkScalarImageKmeansImageFilter );

SoItkScalarImageKmeansImageFilter::SoItkScalarImageKmeansImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkScalarImageKmeansImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( ImageRegion, (0) );
	ImageRegion.setNum(0);
	SO_ENGINE_ADD_INPUT( UseNonContiguousLabels, (false) );
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkScalarImageKmeansImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkScalarImageKmeansImageFilter::checkInput()
{
...
void
SoItkScalarImageKmeansImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::ScalarImageKmeansImageFilter< OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_IMAGE_REGION( filter, ImageRegion, 2 );
						SO_ITK_SET_FIELD_VALUE( filter, UseNonContiguousLabels );
					...
					case 3:
					{
						...
						// Set the filter's inputs
						SO_ITK_SET_FIELD_IMAGE_REGION( filter, ImageRegion, 3 );
						SO_ITK_SET_FIELD_VALUE( filter, UseNonContiguousLabels );
