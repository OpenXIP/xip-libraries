...
SO_ENGINE_SOURCE( SoItkPadImageFilter );

SoItkPadImageFilter::SoItkPadImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkPadImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( PadUpperBound, (0) );
	PadUpperBound.setNum(0);
	SO_ENGINE_ADD_INPUT( PadLowerBound, (0) );
	PadLowerBound.setNum(0);
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkPadImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkPadImageFilter::checkInput()
{
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::PadImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUES( filter, PadUpperBound, unsigned long, 2 );
						SO_ITK_SET_FIELD_VALUES( filter, PadLowerBound, unsigned long, 2 );
					...
					case 3:
					{
						...
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUES( filter, PadUpperBound, unsigned long, 3 );
						SO_ITK_SET_FIELD_VALUES( filter, PadLowerBound, unsigned long, 3 );
