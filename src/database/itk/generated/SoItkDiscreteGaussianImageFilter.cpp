...
SO_ENGINE_SOURCE( SoItkDiscreteGaussianImageFilter );

SoItkDiscreteGaussianImageFilter::SoItkDiscreteGaussianImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkDiscreteGaussianImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( UseImageSpacing, (true) );
	SO_ENGINE_ADD_INPUT( MaximumKernelWidth, (32) );
...
{
	SO_ENGINE_INIT_CLASS( SoItkDiscreteGaussianImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkDiscreteGaussianImageFilter::checkInput()
{
...
void
SoItkDiscreteGaussianImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						SO_ITK_SET_FIELD_VALUE( filter, MaximumKernelWidth );
