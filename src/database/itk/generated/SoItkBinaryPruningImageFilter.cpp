...
SO_ENGINE_SOURCE( SoItkBinaryPruningImageFilter );

SoItkBinaryPruningImageFilter::SoItkBinaryPruningImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkBinaryPruningImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( Iteration, (3) );
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkBinaryPruningImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkBinaryPruningImageFilter::checkInput()
{
...
void
SoItkBinaryPruningImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::BinaryPruningImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUE( filter, Iteration );
