...
SO_ENGINE_SOURCE( SoItkNoiseImageFilter );

SoItkNoiseImageFilter::SoItkNoiseImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkNoiseImageFilter );
...
{
	SO_ENGINE_INIT_CLASS( SoItkNoiseImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkNoiseImageFilter::checkInput()
{
...
void
SoItkNoiseImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::NoiseImageFilter< InputImageType, OutputImageType > FilterType;
