...
SO_ENGINE_SOURCE( SoItkReflectImageFilter );

SoItkReflectImageFilter::SoItkReflectImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkReflectImageFilter );
...
{
	SO_ENGINE_INIT_CLASS( SoItkReflectImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkReflectImageFilter::checkInput()
{
...
void
SoItkReflectImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::ReflectImageFilter< InputImageType, OutputImageType > FilterType;
