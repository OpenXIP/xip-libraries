...
SO_ENGINE_SOURCE( SoItkMultiResolutionPyramidImageFilter );

SoItkMultiResolutionPyramidImageFilter::SoItkMultiResolutionPyramidImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkMultiResolutionPyramidImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( NumberOfLevels, (0) );
	SO_ENGINE_ADD_INPUT( StartingShrinkFactors, (0) );
	SO_ENGINE_ADD_INPUT( Schedule, (0) );
	Schedule.setNum(0);
	SO_ENGINE_ADD_INPUT( MaximumError, (0.1) );
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkMultiResolutionPyramidImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkMultiResolutionPyramidImageFilter::checkInput()
{
...
void
SoItkMultiResolutionPyramidImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::MultiResolutionPyramidImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUE( filter, NumberOfLevels );
						SO_ITK_SET_FIELD_VALUE( filter, StartingShrinkFactors );
						SO_ITK_SET_FIELD_ARRAY2D( filter, Schedule, unsigned int, 2 );
						SO_ITK_SET_FIELD_VALUE( filter, MaximumError );
					...
					case 3:
					{
						...
						SO_ITK_SET_FIELD_VALUE( filter, NumberOfLevels );
						SO_ITK_SET_FIELD_VALUE( filter, StartingShrinkFactors );
						SO_ITK_SET_FIELD_ARRAY2D( filter, Schedule, unsigned int, 3 );
						SO_ITK_SET_FIELD_VALUE( filter, MaximumError );
