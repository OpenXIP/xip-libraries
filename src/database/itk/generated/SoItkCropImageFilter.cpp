...
SO_ENGINE_SOURCE( SoItkCropImageFilter );

SoItkCropImageFilter::SoItkCropImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkCropImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( UpperBoundaryCropSize, (0) );
	UpperBoundaryCropSize.setNum(0);
	SO_ENGINE_ADD_INPUT( LowerBoundaryCropSize, (0) );
	LowerBoundaryCropSize.setNum(0);
}
...
{
	SO_ENGINE_INIT_CLASS( SoItkCropImageFilter, SoItkExtractImageFilter, "SoItkExtractImageFilter" );
}

bool
SoItkCropImageFilter::checkInput()
{
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::CropImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_SIZE( filter, UpperBoundaryCropSize, 2 );
						SO_ITK_SET_FIELD_SIZE( filter, LowerBoundaryCropSize, 2 );
						
						// Inherited from SoItkExtractImageFilter
						SO_ITK_SET_FIELD_IMAGE_REGION( filter, ExtractionRegion, 2 );
					...
					case 3:
					{
						...
						// Set the filter's inputs
						SO_ITK_SET_FIELD_SIZE( filter, UpperBoundaryCropSize, 3 );
						SO_ITK_SET_FIELD_SIZE( filter, LowerBoundaryCropSize, 3 );
						
						// Inherited from SoItkExtractImageFilter
						SO_ITK_SET_FIELD_IMAGE_REGION( filter, ExtractionRegion, 3 );
