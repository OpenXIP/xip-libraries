...
SO_ENGINE_SOURCE( SoItkHConcaveImageFilter );

SoItkHConcaveImageFilter::SoItkHConcaveImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkHConcaveImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( Height, (2) );
...
{
	SO_ENGINE_INIT_CLASS( SoItkHConcaveImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkHConcaveImageFilter::checkInput()
{
...
void
SoItkHConcaveImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::HConcaveImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUE( filter, Height );
