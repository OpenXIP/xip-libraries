...
SO_ENGINE_SOURCE( SoItkSTAPLEImageFilter );

SoItkSTAPLEImageFilter::SoItkSTAPLEImageFilter()
{
	SO_ENGINE_CONSTRUCTOR( SoItkSTAPLEImageFilter );
	
	// Inputs
	SO_ENGINE_ADD_INPUT( ConfidenceWeight, (1.0) );
	SO_ENGINE_ADD_INPUT( MaximumIterations, (65535) );
...
{
	SO_ENGINE_INIT_CLASS( SoItkSTAPLEImageFilter, SoItkImageToImageFilter, "SoItkImageToImageFilter" );
}

bool
SoItkSTAPLEImageFilter::checkInput()
{
...
void
SoItkSTAPLEImageFilter::evaluate()
{
	if( mOutput )
	{
		mOutput->unref();
			...
			case SoItkDataImage::FLOAT:
			{
				switch( Input0.getValue()->getNumDimension() )
				{
					case 2:
					{
						...
						typedef itk::STAPLEImageFilter< InputImageType, OutputImageType > FilterType;
						
						FilterType::Pointer filter = FilterType::New();
						
						// Set the filter's inputs
						SO_ITK_SET_FIELD_VALUE( filter, ConfidenceWeight );
						SO_ITK_SET_FIELD_VALUE( filter, MaximumIterations );
