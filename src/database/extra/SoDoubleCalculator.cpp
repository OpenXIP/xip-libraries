 ...
 *
 ...
 |   $Revision: 1.1.1.1 $
 |
 |   This is the engine mechanism for the SoDoubleCalculator class.  The
...
SO_ENGINE_SOURCE(SoDoubleCalculator);
...
void
SoDoubleCalculator::inputChanged(SoField *which)
{
    if (which == &expression)
	reparse = TRUE;
}
...
double *SoDoubleCalculator::lookupDouble(void *data, const char *nm)
{
    SoDoubleCalculator *e = (SoDoubleCalculator *) data;
    assert(e->isOfType(SoDoubleCalculator::getClassTypeId()));
...
SbVec3f *SoDoubleCalculator::lookupVec3f(void *data, const char *nm)
{
    SoDoubleCalculator *e = (SoDoubleCalculator *) data;
    assert(e->isOfType(SoDoubleCalculator::getClassTypeId()));
    if (strcmp(nm, "A") ==0) return &e->vA;
