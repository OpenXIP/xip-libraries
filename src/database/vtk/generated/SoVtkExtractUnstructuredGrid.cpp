...
# include "SoVtkExtractUnstructuredGrid.h"
...
SO_ENGINE_SOURCE( SoVtkExtractUnstructuredGrid )

SoVtkExtractUnstructuredGrid::SoVtkExtractUnstructuredGrid()
{
	SO_ENGINE_CONSTRUCTOR(SoVtkExtractUnstructuredGrid);

	mObject = vtkExtractUnstructuredGrid::New();
	mObject->Register(0);
	mObject->SetGlobalWarningDisplay(0);

	vtkExtractUnstructuredGrid *aExtractUnstructuredGrid = vtkExtractUnstructuredGrid::New();
	double *x = 0;
	int *y = 0;
	float *z = 0;

	SO_ENGINE_ADD_INPUT(CellClipping, (0));
	CellClipping.setValue(aExtractUnstructuredGrid->GetCellClipping());

	SO_ENGINE_ADD_INPUT(Locator, (0));

	SO_ENGINE_ADD_INPUT(PointClipping, (0));
	PointClipping.setValue(aExtractUnstructuredGrid->GetPointClipping());

	SO_ENGINE_ADD_INPUT(ExtentClipping, (0));
	ExtentClipping.setValue(aExtractUnstructuredGrid->GetExtentClipping());

	SO_ENGINE_ADD_INPUT(Merging, (0));
	Merging.setValue(aExtractUnstructuredGrid->GetMerging());
	...
	aExtractUnstructuredGrid->Delete();
...
void SoVtkExtractUnstructuredGrid::initClass()
{
	SO_ENGINE_INIT_CLASS( SoVtkExtractUnstructuredGrid, SoEngine, "Engine" );
}

void SoVtkExtractUnstructuredGrid::evaluate()
{
	try
	{
...
void SoVtkExtractUnstructuredGrid::inputChanged(SoField * f)
{
...
void SoVtkExtractUnstructuredGrid::reset()
{
	mObject->UnRegister(0);
	mObject->Delete();
	mObject = 0;
