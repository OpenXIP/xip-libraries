...
# include "SoVtkGenericContourFilter.h"
...
SO_ENGINE_SOURCE( SoVtkGenericContourFilter )

SoVtkGenericContourFilter::SoVtkGenericContourFilter()
{
	SO_ENGINE_CONSTRUCTOR(SoVtkGenericContourFilter);

	mObject = vtkGenericContourFilter::New();
	mObject->Register(0);
	mObject->SetGlobalWarningDisplay(0);

	vtkGenericContourFilter *aGenericContourFilter = vtkGenericContourFilter::New();
	double *x = 0;
	int *y = 0;
	float *z = 0;

	SO_ENGINE_ADD_INPUT(NumberOfContours, (0));
	NumberOfContours.setValue(aGenericContourFilter->GetNumberOfContours());

	SO_ENGINE_ADD_INPUT(Value, ());
	Value.setFormat("int float");

	SO_ENGINE_ADD_INPUT(ComputeGradients, (0));
	ComputeGradients.setValue(aGenericContourFilter->GetComputeGradients());

	SO_ENGINE_ADD_INPUT(Locator, (0));

	SO_ENGINE_ADD_INPUT(ComputeScalars, (0));
	ComputeScalars.setValue(aGenericContourFilter->GetComputeScalars());

	SO_ENGINE_ADD_INPUT(ComputeNormals, (0));
	ComputeNormals.setValue(aGenericContourFilter->GetComputeNormals());

	SO_ENGINE_ADD_INPUT(InputConnection, (0));

	aGenericContourFilter->Delete();
...
void SoVtkGenericContourFilter::initClass()
{
	SO_ENGINE_INIT_CLASS( SoVtkGenericContourFilter, SoEngine, "Engine" );
}

void SoVtkGenericContourFilter::evaluate()
{
	try
	{
...
void SoVtkGenericContourFilter::inputChanged(SoField * f)
{
...
void SoVtkGenericContourFilter::reset()
{
	mObject->UnRegister(0);
	mObject->Delete();
	mObject = 0;
