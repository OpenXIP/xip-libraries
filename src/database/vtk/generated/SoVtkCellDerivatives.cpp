...
# include "SoVtkCellDerivatives.h"
...
SO_ENGINE_SOURCE( SoVtkCellDerivatives )

SoVtkCellDerivatives::SoVtkCellDerivatives()
{
	SO_ENGINE_CONSTRUCTOR(SoVtkCellDerivatives);

	mObject = vtkCellDerivatives::New();
	mObject->Register(0);
	mObject->SetGlobalWarningDisplay(0);

	vtkCellDerivatives *aCellDerivatives = vtkCellDerivatives::New();
	double *x = 0;
	int *y = 0;
	float *z = 0;

	SO_ENGINE_ADD_INPUT(TensorMode, (0));
	TensorMode.setValue(aCellDerivatives->GetTensorMode());

	SO_ENGINE_ADD_INPUT(VectorMode, (0));
	VectorMode.setValue(aCellDerivatives->GetVectorMode());

	SO_ENGINE_ADD_INPUT(InputConnection, (0));

	aCellDerivatives->Delete();
...
void SoVtkCellDerivatives::initClass()
{
	SO_ENGINE_INIT_CLASS( SoVtkCellDerivatives, SoEngine, "Engine" );
}

void SoVtkCellDerivatives::evaluate()
{
	try
	{
...
void SoVtkCellDerivatives::inputChanged(SoField * f)
{
	// Get the TensorMode value
	if ( f == &TensorMode )
		SO_VTK_SET_FIELD_VALUE( mObject, TensorMode);
...
void SoVtkCellDerivatives::reset()
{
	mObject->UnRegister(0);
	mObject->Delete();
	mObject = 0;
