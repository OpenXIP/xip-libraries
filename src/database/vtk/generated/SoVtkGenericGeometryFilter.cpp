...
# include "SoVtkGenericGeometryFilter.h"
...
SO_ENGINE_SOURCE( SoVtkGenericGeometryFilter )

SoVtkGenericGeometryFilter::SoVtkGenericGeometryFilter()
{
	SO_ENGINE_CONSTRUCTOR(SoVtkGenericGeometryFilter);

	mObject = vtkGenericGeometryFilter::New();
	mObject->Register(0);
	mObject->SetGlobalWarningDisplay(0);

	vtkGenericGeometryFilter *aGenericGeometryFilter = vtkGenericGeometryFilter::New();
	double *x = 0;
	int *y = 0;
	float *z = 0;

	SO_ENGINE_ADD_INPUT(CellClipping, (0));
	CellClipping.setValue(aGenericGeometryFilter->GetCellClipping());

	SO_ENGINE_ADD_INPUT(Locator, (0));

	SO_ENGINE_ADD_INPUT(PointClipping, (0));
	PointClipping.setValue(aGenericGeometryFilter->GetPointClipping());

	SO_ENGINE_ADD_INPUT(ExtentClipping, (0));
	ExtentClipping.setValue(aGenericGeometryFilter->GetExtentClipping());

	SO_ENGINE_ADD_INPUT(Merging, (0));
	Merging.setValue(aGenericGeometryFilter->GetMerging());
...
void SoVtkGenericGeometryFilter::initClass()
{
	SO_ENGINE_INIT_CLASS( SoVtkGenericGeometryFilter, SoEngine, "Engine" );
}

void SoVtkGenericGeometryFilter::evaluate()
{
	try
	{
...
void SoVtkGenericGeometryFilter::inputChanged(SoField * f)
{
...
void SoVtkGenericGeometryFilter::reset()
{
	mObject->UnRegister(0);
	mObject->Delete();
	mObject = 0;
