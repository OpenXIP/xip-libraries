...
SO_ENGINE_SOURCE( SoVtkClipDataSet )

SoVtkClipDataSet::SoVtkClipDataSet()
{
	SO_ENGINE_CONSTRUCTOR(SoVtkClipDataSet);

	mObject = vtkClipDataSet::New();
	mObject->Register(0);
	mObject->SetGlobalWarningDisplay(0);

	vtkClipDataSet *aClipDataSet = vtkClipDataSet::New();
	double *x = 0;
	int *y = 0;
	float *z = 0;

	SO_ENGINE_ADD_INPUT(Value, (0));
	Value.setValue(aClipDataSet->GetValue());

	SO_ENGINE_ADD_INPUT(InsideOut, (0));
	InsideOut.setValue(aClipDataSet->GetInsideOut());

	SO_ENGINE_ADD_INPUT(GenerateClippedOutput, (0));
	GenerateClippedOutput.setValue(aClipDataSet->GetGenerateClippedOutput());

	SO_ENGINE_ADD_INPUT(Locator, (0));

	SO_ENGINE_ADD_INPUT(GenerateClipScalars, (0));
	GenerateClipScalars.setValue(aClipDataSet->GetGenerateClipScalars());
	...
	MergeTolerance.setValue(aClipDataSet->GetMergeTolerance());
...
void SoVtkClipDataSet::evaluate()
{
	try
	{
...
void SoVtkClipDataSet::inputChanged(SoField * f)
{
	// Get the Value value
	if ( f == &Value )
