...
SO_ENGINE_SOURCE( SoVtkGlyph3D )

SoVtkGlyph3D::SoVtkGlyph3D()
{
	SO_ENGINE_CONSTRUCTOR(SoVtkGlyph3D);

	mObject = vtkGlyph3D::New();
	mObject->Register(0);
	mObject->SetGlobalWarningDisplay(0);

	vtkGlyph3D *aGlyph3D = vtkGlyph3D::New();
	double *x = 0;
	int *y = 0;
	float *z = 0;

	SO_ENGINE_ADD_INPUT(GeneratePointIds, (0));
	GeneratePointIds.setValue(aGlyph3D->GetGeneratePointIds());

	SO_ENGINE_ADD_INPUT(PointIdsName, (""));

	SO_ENGINE_ADD_INPUT(Scaling, (0));
	Scaling.setValue(aGlyph3D->GetScaling());

	SO_ENGINE_ADD_INPUT(Clamping, (0));
	Clamping.setValue(aGlyph3D->GetClamping());

	SO_ENGINE_ADD_INPUT(IndexMode, (0));
	IndexMode.setValue(aGlyph3D->GetIndexMode());

	SO_ENGINE_ADD_INPUT(ScaleMode, (0));
	ScaleMode.setValue(aGlyph3D->GetScaleMode());

	SO_ENGINE_ADD_INPUT(Orient, (0));
	Orient.setValue(aGlyph3D->GetOrient());
	...
	ColorMode.setValue(aGlyph3D->GetColorMode());

	SO_ENGINE_ADD_INPUT(ScaleFactor, (0));
	ScaleFactor.setValue(aGlyph3D->GetScaleFactor());

	SO_ENGINE_ADD_INPUT(InputConnection, (0));

	SO_ENGINE_ADD_INPUT(VectorMode, (0));
	VectorMode.setValue(aGlyph3D->GetVectorMode());
...
void SoVtkGlyph3D::initClass()
{
	SO_ENGINE_INIT_CLASS( SoVtkGlyph3D, SoEngine, "Engine" );
}

void SoVtkGlyph3D::evaluate()
{
	try
	{
...
void SoVtkGlyph3D::inputChanged(SoField * f)
{
